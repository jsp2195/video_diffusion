"""Lightweight proxy metric for video feature Fréchet distance.

This is NOT standard FVD (e.g. I3D/Kinetics-based evaluation). It is a cheap proxy
useful for relative training diagnostics only.
"""
...
class VideoFeatureExtractor:
    ...
    def extract(self, videos):
        ...
        x = (x + 1.0) / 2.0
        x = F.interpolate(x, size=(x.shape[2], 96, 96), mode="trilinear", align_corners=False)
        feat = self.model(x)
...
def frechet_distance(real_feat, gen_feat):
    ...
    diff = mu_r - mu_g
    dist = diff @ diff + np.trace(cov_r + cov_g - 2.0 * cov_mean)
    return float(np.real(dist))


def compute_fvd_proxy(real_videos, gen_videos, device):
