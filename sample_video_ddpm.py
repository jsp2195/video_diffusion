...
def sample(args):
    ...
    if args.T < 3:
        raise ValueError("Resolved T must be >= 3 for endpoint-conditioned middle-frame sampling.")
    if args.frame_stride is None:
        args.frame_stride = int(model_cfg.get("frame_stride", ckpt.get("config", {}).get("frame_stride", 1)))

    if args.start_image and args.end_image:
        cond_start = load_cond_from_image(args.start_image, args.size)
        cond_end = load_cond_from_image(args.end_image, args.size)
    else:
        ...
        cond_start = sample_clip[:, :, 0]
        cond_end = sample_clip[:, :, -1]
    ...
    start_path = os.path.join(out_dir, "start.png")
    end_path = os.path.join(out_dir, "end.png")

    full_clip = torch.cat([cond_start.unsqueeze(2), x, cond_end.unsqueeze(2)], dim=2)
    save_cond_png(cond_start, start_path)
    save_cond_png(cond_end, end_path)
...
def parse_args():
    ...
    p.add_argument("--start_image", type=str, default=None)
    p.add_argument("--end_image", type=str, default=None)
    ...
    if args.start_image is None:
        if args.input_image is not None:
            args.start_image = args.input_image
        elif args.cond_image is not None:
            args.start_image = args.cond_image
    ...
    if not (args.start_image and args.end_image) and args.data_root is None:
        raise ValueError("Provide (--start_image and --end_image) or --data_root")
    return args
