...
def load_cond_sequence(paths, size: int, expected_len: int) -> torch.Tensor:
    if len(paths) != expected_len:
        raise ValueError(f"Expected {expected_len} context frames, got {len(paths)}")
    frames = [load_cond_from_image(p, size) for p in paths]
...
def sample(args):
    ...
    if args.start_images and args.end_images:
        cond_start = load_cond_sequence(args.start_images, args.size, args.endpoint_context)
        cond_end = load_cond_sequence(args.end_images, args.size, args.endpoint_context)
    else:
