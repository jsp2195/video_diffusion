...
class VideoUNetConditional(nn.Module):
    ...
    def _cond_3d_from_feat(self, cond2d: torch.Tensor, target_hw: Tuple[int, int], t_len: int) -> torch.Tensor:
        cond2d = F.interpolate(cond2d, size=target_hw, mode="bilinear", align_corners=False)
    ...
    def forward(self, noisy_clip: torch.Tensor, t: torch.Tensor, cond_first_frame: torch.Tensor) -> torch.Tensor:
        ...
        c0 = self._cond_3d_from_feat(cond2d, (h, w), t_len)
        x = self.in_proj(torch.cat([noisy_clip, c0], dim=1))

        c1 = self._cond_3d_from_feat(cond2d, (x.shape[-2], x.shape[-1]), t_len)
        x1 = self.down1(torch.cat([x, c1], dim=1), t_emb)

        x2_in = self.ds(x1)
        c2 = self._cond_3d_from_feat(cond2d, (x2_in.shape[-2], x2_in.shape[-1]), t_len)
        x2 = self.down2(torch.cat([x2_in, c2], dim=1), t_emb)

        x3_in = self.ds2(x2)
        c3 = self._cond_3d_from_feat(cond2d, (x3_in.shape[-2], x3_in.shape[-1]), t_len)
        x3 = self.down3(torch.cat([x3_in, c3], dim=1), t_emb)

        cmid = self._cond_3d_from_feat(cond2d, (x3.shape[-2], x3.shape[-1]), t_len)
        xm = self.mid(torch.cat([x3, cmid], dim=1), t_emb)

        u1 = self.us1(xm)
        u1 = torch.cat([u1, x2], dim=1)
        cu1 = self._cond_3d_from_feat(cond2d, (u1.shape[-2], u1.shape[-1]), t_len)
        u1 = self.up1(torch.cat([u1, cu1], dim=1), t_emb)

        u2 = self.us2(u1)
        u2 = torch.cat([u2, x1], dim=1)
        cu2 = self._cond_3d_from_feat(cond2d, (u2.shape[-2], u2.shape[-1]), t_len)
