...
def build_model_config(args: argparse.Namespace) -> Dict:
    return {
        ...
        "cond_channels": 3 if args.color_mode == "rgb" else 1,
...
def evaluate_fvd_proxy(ema_model, diffusion, val_ds, args, device):
    ...
    for idx in indices:
        ...
        gen = torch.cat([cond_start_ctx, gen_middle, cond_end_ctx], dim=2)
...
def train(args):
    ...
    if args.T <= 2 * args.endpoint_context:
        raise ValueError("--T must be > 2 * --endpoint_context for middle-frame generation.")
    ...
    train_ds = KineticsVideoDataset(train_files, num_frames=args.T, frame_stride=args.frame_stride, size=args.size, cache_videos=args.cache_videos, color_mode=args.color_mode)
    val_ds = KineticsVideoDataset(val_files, num_frames=args.T, frame_stride=args.frame_stride, size=args.size, cache_videos=args.cache_videos, color_mode=args.color_mode)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in pbar:
            ...
            if args.cfg_drop_prob > 0:
                mask = (torch.rand(cond_start_ctx.shape[0], device=device) < args.cfg_drop_prob).float().view(-1, 1, 1, 1, 1)
                keep = (1.0 - mask)
                cond_start_ctx = cond_start_ctx * keep
                cond_end_ctx = cond_end_ctx * keep

            t = diffusion.sample_timesteps(cond_start_ctx.shape[0], device)
        ...
        if is_main_process():
            ...
            if (epoch + 1) % args.vis_every == 0 and fixed_idx is not None:
                ...
                preview_video = torch.cat([preview_start_ctx, preview_middle, preview_end_ctx], dim=2)
...
def build_parser():
    ...
    p.add_argument("--endpoint_context", type=int, default=2)
